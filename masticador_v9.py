...
with dc1:
    ...
    fig.update_layout(**pl({
        ...
        "height":340,
        "showlegend":True,
        "legend":dict(font=dict(color=C["text"],size=12),orientation="h",y=-0.08),
    }))
    fig.add_annotation(
        text=f"<b>{asr_global:.0f}%</b><br>ASR GLOBAL",
        x=0.5, y=0.5, showarrow=False, align="center",
        font=dict(family="Orbitron", size=16,
...
with dc2:
    ...
    fig.add_annotation(
        text=f"<b>{int(mob_pct)}%</b><br>MÓVIL",
        x=0.5, y=0.5, showarrow=False,
        font=dict(family="Orbitron", size=16, color=C["purple"]))
    st.plotly_chart(fig, use_container_width=True)

# ── TOP RELEASE CAUSES: debajo de las donas, ancho completo ──────────
section("TOP RELEASE CAUSES")
...
fig = go.Figure(go.Bar(
    ...
    textposition="outside", textfont=dict(color="#ffffff",size=11,family="Rajdhani"),
    hovertemplate="<b>%{y}</b><br>%{x:,} llamadas<extra></extra>",
))
fig.update_layout(**pl({
    "height":360,
    "title":dict(text="Top Release Causes  — distribución por tipo de corte",font=dict(color=C["cyan"],size=13)),
...
# ── Distribución completa con heatmap de calor verde ─────────────────
section("DISTRIBUCIÓN COMPLETA DE RELEASE CAUSES")
tbl_causa = causa_cnt[["causa","nombre","desc","cat","count","pct"]].copy()
tbl_causa.columns = ["Código","Nombre técnico","Descripción","Categoría","Llamadas","%"]
tbl_causa["Llamadas_fmt"] = tbl_causa["Llamadas"].apply(lambda v: f"{v:,}".replace(",","."))

# Build HTML heatmap table
max_calls = tbl_causa["Llamadas"].max()
rows_html = ""
for _, r in tbl_causa.iterrows():
    intensity = r["Llamadas"] / max_calls
    green_alpha = max(0.06, intensity * 0.55)
    cat_color = {"normal":"#00ff88","congestion":"#ff6b35","error":"#ff2d55",
                 "warning":"#ffcc00","packet_loss":"#a855f7","other":"#3a7ca5"}.get(r["Categoría"],"#3a7ca5")
    rows_html += f"""<tr style="background:rgba(0,255,100,{green_alpha:.2f});">
      <td style="padding:6px 10px;font-family:Orbitron,monospace;font-size:0.75rem;color:#00d4ff;text-align:center;">{r["Código"]}</td>
      <td style="padding:6px 10px;font-family:Share Tech Mono,monospace;font-size:0.72rem;color:#a0cce0;">{r["Nombre técnico"]}</td>
      <td style="padding:6px 10px;font-family:Rajdhani,sans-serif;font-size:0.85rem;color:#e0f4ff;font-weight:600;">{r["Descripción"]}</td>
      <td style="padding:6px 10px;text-align:center;"><span style="background:{cat_color}22;color:{cat_color};border:1px solid {cat_color};border-radius:4px;padding:2px 8px;font-family:Share Tech Mono;font-size:0.68rem;">{r["Categoría"]}</span></td>
      <td style="padding:6px 10px;font-family:Orbitron,monospace;font-size:0.8rem;color:#ffffff;text-align:right;font-weight:700;">{r["Llamadas_fmt"]}</td>
      <td style="padding:6px 10px;font-family:Share Tech Mono,monospace;font-size:0.78rem;color:#00ff88;text-align:right;">{r["%"]}%</td>
    </tr>"""

causa_table_html = f"""
<div style="overflow-x:auto;border:1px solid #0d3a5e;border-radius:8px;margin-bottom:8px;">
<table style="width:100%;border-collapse:collapse;background:#020f1c;">
  <thead>
    <tr style="background:#0d3a5e;">
      <th style="padding:8px 10px;font-family:Orbitron,monospace;font-size:0.7rem;color:#00d4ff;letter-spacing:2px;text-align:center;">CÓDIGO</th>
      <th style="padding:8px 10px;font-family:Orbitron,monospace;font-size:0.7rem;color:#00d4ff;letter-spacing:2px;">NOMBRE Q.850</th>
      <th style="padding:8px 10px;font-family:Orbitron,monospace;font-size:0.7rem;color:#00d4ff;letter-spacing:2px;">DESCRIPCIÓN</th>
      <th style="padding:8px 10px;font-family:Orbitron,monospace;font-size:0.7rem;color:#00d4ff;letter-spacing:2px;text-align:center;">CATEGORÍA</th>
      <th style="padding:8px 10px;font-family:Orbitron,monospace;font-size:0.7rem;color:#00d4ff;letter-spacing:2px;text-align:right;">LLAMADAS</th>
      <th style="padding:8px 10px;font-family:Orbitron,monospace;font-size:0.7rem;color:#00d4ff;letter-spacing:2px;text-align:right;">%</th>
    </tr>
  </thead>
  <tbody>{rows_html}</tbody>
</table>
</div>"""
st.markdown(causa_table_html, unsafe_allow_html=True)
...
# ── Bar chart — wider, white labels, dark-green ASR Real line ────────
ASR_REAL_GREEN = "#00994d"   # dark green for ASR Real line
...
fig.add_trace(go.Scatter(
    ...
    textfont=dict(size=9, color=C["yellow"], family="Share Tech Mono"),
), secondary_y=True)
fig.add_trace(go.Scatter(
    name="ASR Real %",
    ...
    marker=dict(size=7, color=ASR_REAL_GREEN,
                line=dict(color="#ffffff",width=1)),
    text=dest_top["asr_real_d"].round(1).astype(str)+"%",
    textposition="bottom center",
    textfont=dict(size=9, color=ASR_REAL_GREEN, family="Share Tech Mono"),
), secondary_y=True)
fig.add_hline(y=50, line_dash="dot", line_color=C["orange"],
              annotation_text="50% ASR", annotation_font_color=C["orange"],
              secondary_y=True)
fig.update_xaxes(
    tickfont=dict(size=11, color="#ffffff", family="Rajdhani"),
    tickangle=-40,
    gridcolor="#0d3a5e", zerolinecolor="#0d3a5e",
)
...
# ── Tabla estilo reference image ───────────────────────────────────────
def asr_dots(val):
    """Return colored dots like the reference image based on ASR value."""
    if val >= 70:   color, label = "#00ff88", "●●●"
    elif val >= 40: color, label = "#ffcc00", "●●○"
    else:           color, label = "#ff6b35", "●○○"
    return f'<span style="color:{color};font-size:1rem;">{label}</span> <b style="color:{color}">{val:.0f}%</b>'

def bar_html(val, max_val, color="#00d4ff"):
    pct = min(val/max_val*100, 100) if max_val > 0 else 0
    return (f'<div style="display:flex;align-items:center;gap:6px;">'
            f'<div style="background:{color};height:8px;width:{pct:.0f}%;'
            f'min-width:4px;border-radius:4px;max-width:120px;"></div>'
            f'<span style="font-family:Rajdhani;font-size:0.88rem;color:#ffffff;font-weight:700;">'
            f'{val:,}'.replace(",",".") + f'</span></div>')

max_calls_d = dest_top["llamadas"].max()
max_min_d   = dest_top["minutos"].max()
...
for i,(_, r) in enumerate(dest_top.iterrows(), 1):
    ...
    rows_html += (
        f'<tr style="background:{bg};border-bottom:1px solid #0d3a5e;">' +
        f'<td style="padding:8px 10px;font-family:Orbitron,monospace;font-size:0.72rem;color:#3a7ca5;text-align:center;">{i}</td>' +
        f'<td style="padding:8px 14px;font-family:Rajdhani,sans-serif;font-size:0.95rem;color:#ffffff;font-weight:700;">{r["dest_nombre"]}</td>' +
        f'<td style="padding:8px 10px;font-family:Share Tech Mono,monospace;font-size:0.72rem;color:#3a7ca5;text-align:center;">{r["dest_modal"]}</td>' +
        f'<td style="padding:8px 10px;">{asr_dots(r["asr_real_d"])}</td>' +
        f'<td style="padding:8px 10px;">{bar_html(r["llamadas"], max_calls_d)}</td>' +
        f'<td style="padding:8px 10px;font-family:Rajdhani,sans-serif;font-size:0.88rem;color:#c8e6ff;text-align:right;">{conn_fmt}</td>' +
        f'<td style="padding:8px 10px;font-family:Orbitron,monospace;font-size:0.78rem;color:#ffcc00;text-align:right;font-weight:700;">{min_fmt}</td>' +
        f'<td style="padding:8px 10px;font-family:Share Tech Mono,monospace;font-size:0.75rem;color:#00d4ff;text-align:center;">{r["asr_global_d"]:.1f}%</td>' +
        '</tr>'
    )

dest_table_html = (
    '<div style="overflow-x:auto;border:1px solid #0d3a5e;border-radius:8px;margin-top:4px;">'
    '<table style="width:100%;border-collapse:collapse;background:#020f1c;">'
    '<thead><tr style="background:#0a2540;border-bottom:2px solid #00d4ff;">'
    '<th style="padding:10px 10px;font-family:Orbitron,monospace;font-size:0.68rem;color:#00d4ff;letter-spacing:2px;text-align:center;">N°</th>'
    '<th style="padding:10px 14px;font-family:Orbitron,monospace;font-size:0.68rem;color:#00d4ff;letter-spacing:2px;">DESTINO</th>'
    '<th style="padding:10px 10px;font-family:Orbitron,monospace;font-size:0.68rem;color:#00d4ff;letter-spacing:2px;text-align:center;">MODALIDAD</th>'
    '<th style="padding:10px 10px;font-family:Orbitron,monospace;font-size:0.68rem;color:#00d4ff;letter-spacing:2px;">ASR REAL (%)</th>'
    '<th style="padding:10px 10px;font-family:Orbitron,monospace;font-size:0.68rem;color:#00d4ff;letter-spacing:2px;">TOTAL CALLS</th>'
    '<th style="padding:10px 10px;font-family:Orbitron,monospace;font-size:0.68rem;color:#00d4ff;letter-spacing:2px;text-align:right;">CALLS OK</th>'
    '<th style="padding:10px 10px;font-family:Orbitron,monospace;font-size:0.68rem;color:#ffcc00;letter-spacing:2px;text-align:right;">MINUTOS</th>'
    '<th style="padding:10px 10px;font-family:Orbitron,monospace;font-size:0.68rem;color:#00d4ff;letter-spacing:2px;text-align:center;">ASR GLOBAL</th>'
    '</tr></thead>'
    f'<tbody>{rows_html}</tbody>'
    '</table></div>'
)
st.markdown(dest_table_html, unsafe_allow_html=True)
...
with cr1:
    ...
    # Detect outliers: values > 5x median get flagged
    ...
    if use_log:
        st.markdown(
            f'<div style="font-family:Share Tech Mono;font-size:0.7rem;color:#ffcc00;">'
            f'⚠ Escala logarítmica — outlier detectado: '
            f'<b>{top20["num_b"].iloc[0]}</b> con <b>{top20["intentos"].iloc[0]:,}</b> intentos</div>',
            unsafe_allow_html=True)
    st.plotly_chart(fig, use_container_width=True)
with cr2:
